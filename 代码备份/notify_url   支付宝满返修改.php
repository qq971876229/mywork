	...
	if ($_POST ['trade_status'] == 'TRADE_FINISHED') {
		...
		if (is_array ( $rows )) {
			...
			while ( false !== ($row = DB::fetch ( $queryb )) ) {
				...
				$nums_rst = DB::query ( "Update  `ymy_product` Set nums=nums-$procount,soldnum=soldnum+$procount  WHERE  `id`=$proid and nums>0" );
				if (! $nums_rst) {
					echo 'fail';
					exit ();
				}
			...
			// 自己购买返回积分
			writeilogs ( "购买商品", "商品订单号[" . $rows ["void"] . "]", floor ( $rows ["amount"] * (sys_payjf / 100) ), $rows ["username"] );
		...
		if (is_array ( $rows )) {
			...
			/*白名单中返回蚂蚁盾*/
			$fullreturn_white='SELECT sum,back FROM '.DB::table('fullreturn').' WHERE isdel=0 AND checked=1 AND (starttime<'.YMY_NOW_TIME.' AND endtime>'.YMY_NOW_TIME.') AND sum<='.$white_amount.' order by sum desc';
			...
			/*除去黑白名单剩余产品返回蚂蚁盾*/
			$fullreturn_white='SELECT sum,back FROM '.DB::table('fullreturn').' WHERE isdel=0 AND checked=1 AND (starttime<'.YMY_NOW_TIME.' AND endtime>'.YMY_NOW_TIME.') AND sum<='.$rows['amount'].' order by sum desc';
			$fullreturn=DB::fetch_first($fullreturn_white);
			file_put_contents('D:/fan.txt', json_encode($fullreturn)."\r\n\r\n");
			if(!empty($fullreturn)){
				writemydlogs ( "购买商品", "满" . $rows['amount'] . "送" . $fullreturn['back'] . ":订单号[" . $rows ["void"] . "]", $fullreturn['back'], $rows["username"] );			
