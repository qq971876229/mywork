...
//提交
if(YMY_IS_POST){
	if($action == 'add' || $action == 'edit_tuan'){
		...
		$product_sql = 'select * from '.DB::table('tuan_app').' where id='.$tuanid;
		$product = DB::fetch_first($product_sql);
		//echo $product_sql;
		if(empty($product)){
			FormatMsg('商品不存在，无法设置团购<br /><a href="index_tuan.php">返回团购列表</a>');
		}
		if($product['isdel'] > 0){
			FormatMsg('商品已删除，无法设置团购<br /><a href="index_tuan.php">返回团购列表</a>');
		}
		if($product['checked'] < 1){
			FormatMsg('商品未通过审核，无法设置团购<br /><a href="index_tuan.php">返回团购列表</a>');
		}
		/* if($product['state'] < 1){
			FormatMsg('商品未上架，无法设置团购<br /><a href="index_tuan.php">返回团购列表</a>');
		} */
		...
		if($action == 'add'){
			$tuan_sql = 'select id from '.DB::table('tuan_app').' where productid='.$productid.' and ((starttime<'.YMY_NOW_TIME.' and endtime>'.YMY_NOW_TIME . ') or starttime>' . YMY_NOW_TIME . ') and isdel=0';
		...
		if($action == 'add'){
			...
			$result = DB::insert('tuan_app', $data, true);
			$action_type = '添加';
		}
		elseif($action == 'edit_tuan'){
			$data['editmaster'] = YMY_NOW_MASTER;
			$data['edittime'] = YMY_NOW_TIME;
			$condition = array('id' => $tuanid);						
			$result = DB::update('tuan_app', $data, $condition);			
			$action_type = '编辑';
		}
		
		if($result){
			$msg = '团购'.$action_type.'成功<br /><a href="index_tuan.php">返回团购列表</a>';
			FormatMsg($msg);
		}
		else{
...
if($action == 'add' || $action == 'edit_tuan'){
	if($action == 'add'){
		...
		$tuan_sql = 'select id,productid,bigclassid,smallclassid,price,tprice,title,description,content,discount,starttime,endtime,posttime,checked,sortorder from '.DB::table('tuan_app').' where productid='.$productid.' and ((starttime<'.YMY_NOW_TIME.' and endtime>'.YMY_NOW_TIME.') or starttime>'.YMY_NOW_TIME.')  and isdel=0';
	}
	else if($action == 'edit_tuan'){
		$tuanid = Q('tuanid');
		...
		$tuan_sql = 'select id,productid,productnumber,bigclassid,smallclassid,price,tprice,title,description,content,images,discount,starttime,endtime,posttime,checked,sortorder from '.DB::table('tuan_app').' where id='.$tuanid.' and isdel=0';
	}
	
	
	$tuan = DB::fetch_first($tuan_sql);
	if($action == 'edit_tuan' && empty($tuan)){
	...
	$product_sql = 'select *  from '.DB::table('tuan_app').' where productid='.$productid;
	$product = DB::fetch_first($product_sql);	
	if(empty($product)){
		FormatMsg('商品不存在，无法设置团购<br /><a href="index_tuan.php">返回团购列表</a>');
	}
...
if($action != 'add' && $action !='edit_tuan'){
	if($action == 'del'){
		$del_id = intval( Q('id') );
		$del_sql = 'update '.DB::table('tuan_app').' set isdel=1,checked=0,deltime='.YMY_NOW_TIME.',delmaster=\''.YMY_NOW_MASTER.'\' where id='.$del_id.' limit 1';
	...
	$list_sql = 'select '.implode(',', $list_fields).' from '.DB::table('tuan_app').' where isdel=0 ';
	
	if(!empty($keyword)){
		$list_sql .= ' and (producttitle like \'%'.$keyword.'%\' or productnumber like \'%'.$keyword.'%\' or title like \'%'.$keyword.'%\') ';
	}
	
	if($smallclassid > 0){
		$list_sql .= ' and (smallclassid=\''.$smallclassid.'\' or bigclassid like \'%|'.$smallclassid.'|%\') ';
	}
	elseif($bigclassid > 0){
		$list_sql .= ' and (smallclassid=\''.$bigclassid.'\' or bigclassid like \'%|'.$bigclassid.'|%\') ';
	}
	elseif($classid > 0){
		$list_sql .= ' and (smallclassid=\''.$classid.'\' or bigclassid like \'%|'.$classid.'|%\') ';
	}
	
	if(!empty($checked)){
		if($checked == 99){
			$list_sql .= ' and checked=1 ';
		}
		else{
			$list_sql .= ' and checked=0 ';
		}
	}
...
switch($action){
	case 'add': // 添加
		include 'edit_tuan.php';
		break;
	case 'edit_tuan': // 编辑
		include 'edit_tuan.php';
		break;
	case 'del': // 删除
		include 'list_tuan.php';
		break;
	default: // 列表
		include 'list_tuan.php';
}
